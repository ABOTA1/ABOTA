...
from app.db.queries import get_genre_breakdown, get_mentions_trend, get_top_movies_by_revenue
...
def test_top_movies_include_cross_metric_mentions(mock_exec):
    ...
    assert "social_mentions" in sql
    assert "content_catalog" in sql
    assert "c.genre" in sql
...
@patch("app.db.queries.execute_query")
def test_genre_breakdown_joins_content_catalog(mock_exec):
    mock_exec.return_value = [{"genre": "Sci-Fi", "titles": 4, "total_revenue": 9_000_000}]
    rows = get_genre_breakdown()
    sql = mock_exec.call_args[0][0]
    assert "content_catalog" in sql
    assert "c.genre" in sql
    assert rows[0]["genre"] == "Sci-Fi"
